jQuery(document).ready(function ($) {
  ...
  var win = $(window),
    ...
    renderer = webGl ? new THREE.WebGLRenderer({
      antialias: true
    ...
    material = new THREE.MeshBasicMaterial({
      color: white
    }), // The normal material used to display hands and fingers
    recordingMaterial = new THREE.MeshBasicMaterial({
      color: yellow
    }), // The material used on hands and fingers during recording
   ...
   */
  optionsButton.click(function (evt) {
    optionsOpen ? closeOptions(evt) : openOptions(evt);
  });
  ...
  function setupOptionList(rt, t, list, implName) {
    if (rt) {
      ...
      if (t.indexOf(rt) == -1) {
        t.push(rt);
        list.append('<option value="' + implName + '">' + rt + '</option>');
      }
    }
  }
  ...
  function openExportOverlay(listItem, gestureName) {
    ...
    if (overlayOpen || training) {
      return;
    } // If a gesture is currently in training, the overlay can't be opened
    ...
    localStorage.setItem('dat', JSON.stringify(json));
    ...
    exportText.css({
      height: overlayArea.height() - (overlayArea.children()[0].clientHeight + 150)
    });
  };
  ...
  overlayShade.on('click', function (e) {
    if (body.hasClass('overlay-open')) {
      closeExportOverlay();
    }
  }); // Clicking anywhere on the overlay shade closes the overlay

  $(document).on('keydown', function (e) {
    if (e.keyCode === 27) {
      closeExportOverlay();
    }
  }); // Pressing the ESC key closes the overlay
  ...
  function updateDimensions() {
    ...
    exportText.css({
      height: overlayArea.height() - (overlayArea.children()[0].clientHeight + 150)
    }); // The export textarea stretches vertically
  }
  ...
  newGestureName.focus(function () {
    ...
    if ($(this).val() == "") $(this).val($(this).data('default-text'));
  });
  ...
  creationForm.submit(function () {
     ...
     */
    if (name.length == 0 || name == newGestureName.data("default-text") || trainer.gestures[name] != null) {
      return false;
    }
     ...
     * 
     * The gesture name is upper-cased for uniformity.
     */
  ...
  function setGestureScale(gestureName, val, color, arrowColor) {
    ...
    gestureArrows[gestureName].css({
      display: arrowColor == 'transparent' ? 'none' : 'block',
  ...
  function registerGesture(gestureName, trainingSkipped) {
    ...
    gesture.click(function () {
      openExportOverlay(gesture, gestureName);
  ...
  trainer.on('training-started', function (gestureName) {
    ...
    gestureEntries[gestureName].css({
      background: 'transparent'
    });
  ...
  controller.on('connect', function () {
    setOutputText('Create a gesture or pose to get started');
  });
  ...
  controller.on('deviceDisconnected', function () {
    disableUI(red, 'DISCONNECTED!  Check the connection to your Leap Motion');
  });
  ...
  trainer.on('started-recording', function () {
      setHandMaterial(recordingMaterial);
    })
  ...
  /*
   * WHY is it necessary to offset mesh positions on the y-axis by 170? I don't know at the moment - but this bit of nonsense should be fixed.
   */
  var yOffset = -170;
  ...
  function clearGesture() {
    ...
    for (var i = 0, l = renderedHands.length; i < l; i++) {
      scene.remove(renderedHands[i]);
    }
  ...
  function renderGesture() {
    ...
    if (!webGl) {
      return;
    } // Gesture renders are entirely disabled for canvas renderers (it's just too slow at the moment!)
    ...
    if (!gestureFrames || gestureFrames.length == 0) {
      return;
    } // If the controller doesn't supply this variable, or the array is empty, we return.
